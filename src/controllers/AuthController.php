...
class AuthController extends Controller
{
    ...
    public function facebook(Request $request)
    {
        ...
        if ($request->header('Authorization')) {
            ...
            $payload = (array)JWT::decode($token, Config::get('app.token_secret'), ['HS256']);
    ...
    public function twitter(Request $request)
    {
        ...
        if (!$request->input('oauth_token') || !$request->input('oauth_verifier')) {
            ...
            if ($request->header('Authorization')) {
                ...
                if ($user->first()) {
                    return response()->json(['message' => 'There is already a Twitter account that belongs to you'],
                        409);
                }
    ...
    public function foursquare(Request $request)
    {
        ...
        if ($request->header('Authorization')) {
            ...
            if ($user->first()) {
                return response()->json(['message' => 'There is already a Foursquare account that belongs to you'],
                    409);
            }
    ...
    public function instagram(Request $request)
    {
        ...
        if ($request->header('Authorization')) {
            ...
            if ($user->first()) {
                return response()->json(['message' => 'There is already an Instagram account that belongs to you'],
                    409);
            }
